...
def main():
	...
	if params.run != "GENDIST":
		K = ast.parseSubgraphFromPoints(params, point_coords, pop_coords, G)
	...
	if params.run in ["STREAMDIST", "DISTANCES", "STREAMTREE", "IBD", "ALL", "RUNLOCI"]:
		...
		(sdist, inc) = ast.getStreamMats(points, K, params.length_col)
		print("\nStream distances:")
		print(sdist)
		...
		if params.run in ['ALL', 'IBD']:
			...
			ast.testIBD(gen, sdist, params.out, params.permutations)

			#plot geographic x genetic DISTANCES
			ast.plotGenByGeo(gen, sdist, params.out)
			...
			if params.and_log:
				...
				out=params.out+".log"
				ast.testIBD(gen, sdist, out, params.permutations, log=True)
				ast.plotGenByGeo(gen, sdist, out, log=True)
	...
	if params.run in ["STREAMTREE", "ALL", "RUNLOCI"]:
		...
		if params.run != "RUNLOCI":
			print()
			R = ast.fitLeastSquaresDistances(gen, inc.astype(int), params.iterative, params.out,params.weight)
		...
		if params.run == "RUNLOCI":
			R=averageR
		pred=ast.getFittedD(points, gen, inc, R)
		print("\nComparing observed versus predicted genetic distances:")
		print(pred)
		ast.outputFittedD(pred, params.out)

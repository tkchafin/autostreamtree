...
def dbscan_cluster(coords: Dict[str, Tuple[float, float]], epsilon: float, 
    ...
    Parameters:
    coords (Dict[str, Tuple[float, float]]): A dictionary of coordinate pairs with 
                                             the key as the location identifier and 
                                             values as tuples of (latitude, longitude).
    epsilon (float): Maximum distance between two samples for them to be considered 
    ...
    Returns:
    SortedDict: A sorted dictionary of cluster labels with keys as cluster name and 
                values as lists of location identifiers in that cluster.

    Raises:
    ValueError: If any coordinate value is missing or not a float.

    Examples:
    >>> coords = {'A': (33.4484, -112.0740), 'B': (37.7749, -122.4194), 'C': (40.7128, -74.0060)}
    ...
    for k, v in coords.items():
        if not isinstance(v, tuple) or len(v) != 2:
            raise ValueError(f"Coordinate value for '{k}' is missing or not a tuple of size 2.")
        if not all(isinstance(coord, float) for coord in v):
            raise ValueError(f"Coordinate value for '{k}' has non-float values.")
...
def coords_to_matrix(coords):
    ...
    Args:
    - coords (dict): A dictionary containing the coordinates in the format {id:(long, lat)}.
...
def get_cluster_centroid(coords, popmap, out=None):
    ...
    for i, pop in enumerate(list(popmap.keys())):
        ...
        #print(cluster)
        if len(cluster)<1:
            print("ERROR: get_cluster_centroid(): No coordinates in cluster:",pop)
            sys.exit(1)
